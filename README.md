...
> **v1.0.0 更新**: 新增深色模式、流式引用标注、多轮对话上下文、HyDE 查询扩展、代码智能分块、API 连接测试、重复文档检测等 15 项重大改进。
...
│   │   ├── routers/
│   │   │   ├── notebooks.py    # 笔记本 CRUD API + 笔记管理
│   │   │   ├── documents.py    # 文档上传/URL抓取 + 重复检测 + 内容预览
│   │   │   └── chat.py         # RAG 对话 + SSE 流式 + 多轮上下文
│   │   ├── services/
...
│   │   │           └── page.tsx # 笔记本详情页
│   │   ├── components/
│   │   │   ├── Sidebar.tsx        # 左侧笔记本列表 + 深色模式切换
│   │   │   ├── SourcePanel.tsx    # 文档源面板
│   │   │   ├── UploadZone.tsx     # 拖拽上传 + URL抓取
│   │   │   ├── ChatPanel.tsx      # 聊天面板 + 流式引用 + 多轮对话
│   │   │   ├── NoteEditor.tsx     # 笔记编辑器 (服务端持久化)
│   │   │   ├── SettingsDialog.tsx # API 设置弹窗 + 连接测试
